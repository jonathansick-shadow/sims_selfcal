...
def patchPlots(truepatchfile='patchdata.dat', bestfitpatchfile = "test_bestfit_Patch.dat"):
    ...
    cp.histogram_patch_dmagcaltrue(patchdat, nbins=nbins, range=histrange,
                                   etitle=etitle+"RMS %4.2f mmag"%(1000.*rms))
    figname = "Pdmag_hist."
    pyl.savefig(figname + figformat, format=figformat)
    ...
    output['Photometric Fraction'] = float(
        np.size(np.where(patchdat['dmagcal']-np.min(patchdat['dmagcal']) < .01)))/np.size(patchdat['dmagcal'])
    pyl.title('Extinction Distribution')
    pyl.text(.4, .8, 'Photometric Fraction = %3.3f' %
             output['Photometric Fraction'], transform=pyl.gca().transAxes)
...
def starPlots(truestarfile = "stardata.dat", bestfitstarfile = "test_bestfit_Star.dat"):
    ...
    if HP:
        ...
        zlim = [np.min(stardat['nobs']), (np.median(stardat['nobs']) -
                                          np.min(stardat['nobs']))+np.median(stardat['nobs'])]
