...
Based on Michel's Makecard2A.py, modified to class structure & deal with single
run at a time, while removing hard-coded paths.

This class provides methods to translate atmospheric parameter inputs (generated
by modtranSequences) into the MODTRAN input .tp5 files, including the optional
Card 2A (for cirrus clouds) and the (required for us) card 2B to specify aerosol
features.  The methods for writing these cards are based on code written by
Michel Creze. In MODTRAN speak, a 'card' is the information related to a single
entry for MODTRAN (and is usually a 'line'). What Michel called a 'rank' can be
translated to which 'line' this card is on in the .tp5 file.  The entire tp5
(tape5) file is a series of 'cards'.

The class also provides a method to spawn a shell process to run MODTRAN
(assuming you have the external MODTRAN program and license).

The file Cardtemplate.dat provides the base template for the MODTRAN .tp5 file,
and is kept in ${ATMOSPHERES_TRANSMISSION_DIR}/data. The file
ParameterFormats.dat specifies information on the parameters which can be
changed in the .tp5 file (at least, what can be changed using the parameters
...
class ModtranCards(object):
    ...
    def readCardTemplate(self, templatefile=None):
        ...
        This has the benefit that parameters which are not changed by our setup
        are left in the default state."""
        # (although, the string-slicing that results may not be the easiest
        # thing to read)
        ...
        self._cardTemplate = []
        with open(templatefile, 'r') as tempf:
            for line in tempf:
                self._cardTemplate.append(line)
        # _cardTemplate is a list, with each line from the template file as a
    ...
    def readParameterFormats(self, formatfile=None):
        ...
        # _paramFormats is a dictionary, where each entry is keyed to a
        # parameter that can be changed in the .tp5 file. Each dictionary
        # entry is itself a dictionary, storing the line ('outputLine') in the
        # MODTRAN .tp5 file where the data will be changed, along with the
        # starting and ending character values ('startChar', 'endChar') and the
    ...
    def readParamValues_M(self, parameterfile):
        ...
        for line in file:
            paramValuesDict = {}
            # The current format version of Michel's parameter file is values
            # separated by $, with additional spaces on either side of entries
            # like 'ID85144469 $ expMJD = 49353.165757 $ fieldRA = 0.793502..'
    ...
    def readParamValues_F(self, parameterfile):
        ...
        for line in file:
            if line.startswith('#'):
                ...
                continue
            if not keys:
                raise Exception('Could not get dictionary keys from file \
                    header line {0}'.format(line))
            paramValuesDict = {}
            values = line.split()
    ...
    def _validateParamValues(self, paramValuesDict):
        """Given a dictionary of parameters which will be changed for
        ...
        for parname in paramValuesDict.keys():
            if parname in self._paramFormats.keys():
                ...
                paramValuesDict[parname] = self._paramFormats[parname]['type'](
                    paramValuesDict[parname])
        return keys_used
        ...
        for paramValuesDict in paramValues:
            ...
            for key in paramkeys:
                ...
                card[line] = card[line][:(self._paramFormats[key]['startChar']-1)] + \
                    ...
                    card[line][(self._paramFormats[key]['endChar']):]
    ...
    def cleanModtran(self, outfileRoot='tmp'):
        """Spawn a shell process to clean up all modtran files starting
        with outfileRoot."""
        # MODTRAN suffixes:
