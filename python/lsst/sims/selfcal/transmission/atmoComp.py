...
A class designed to read modtran atmospheric component files, investigate various components of the 
atmosphere and how they scale with airmass, and also build atmospheres with variable components.

This does not do the same thing as MODTRAN itself, but can be a useful tool when a less-accurate 
atmosphere is good enough. I believe this is basically the method David Burke uses to generate 
fits to observed atmospheres (see the equations used to generate S_atm in the calib doc & elsewhere). 

Class data:
 atmo_ind (list of the individual atmospheric absorption features)
 atmo_templates (templates of the individual atmospheric absorption features) - dictionaries,
     keyed airmass / comp
 wavelength 
 total transmission

Many of the methods here deal with plotting the atmosphere or atmospheric components, as this
class was originally used to generate plots for the calibration document. The first few methods 
...
class AtmoComp:
    ...
    def readModtranFiles(self, dataDir=None, dataRoot="Pachon_MODTRAN", dataSuffix=".7sc"):
        ...
        atmofiles = []
        for f in filenames:
            if (f.startswith(dataRoot)) & (f.endswith(dataSuffix)):
                atmofiles.append(f)
        ...
        for f in atmofiles:
            ...
            # files have 2 header lines which include info related to running modtran.
            ...
            # fix aerosols to somewhat realistic values, but
            #  note this will be different than a proper modtran run!
            ...
            self.atmo_templates[z] = deepcopy(templates)
            self.atmo_trans[z] = deepcopy(trans)
