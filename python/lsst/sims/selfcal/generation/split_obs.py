...
def galac_b(ra, dec):
    ...
    result = np.arcsin(np.sin(dec)*np.cos(62.87*np.pi/180.)-np.cos(dec) *
                       np.sin(ra-282.86*np.pi/180.)*np.sin(62.87*np.pi/180))
    return result
...
def split_obs(nside, shift_size=5., splitNS=False, NorthOnly=False, SouthOnly=False, blocksize=1e6):
    ...
    heal['b'] = galac_b(heal['ra'], heal['dec'])
    ...
    # ok, just go through and set duplicates to -1.  Also set any of the
    # shifted ones that are not in patch_healpix to -1
    ...
    keys = [patch_healpix, patch_healpix_shifted1, patch_healpix_shifted2,
    ...
    # I could also just loop through each HP.  if the HP has patches w/ b> 0
    # and b< 0 and min l < xxx or max l < xxx, then break it apart.
    if splitNS:
        break_up = np.where((patchdat['b'] < 0) & ((patchdat['l'] < 120.*np.pi/180.)
                                                   | (patchdat['l'] > 240)*np.pi/180.) & (np.abs(patchdat['b']) < 5*np.pi/180.))
        for key in keys:
            mask = np.where(key == -1)
...
def run_solver(solver, cpu_count=None):
    ...
    for i in np.arange(n):
        commands.append('%s %s %s 1e-12 40000 > /dev/null'%(solver, files[i], files[i][0:-12]))
...
def patch_combine():
    ...
    for obj in range(0, len(patchdat['patchid'])):
        print >>file, "%d  %d  %f  %f %d"%(patchdat['healid'][obj], patchdat['patchid'][
                                           obj], patchdat['patchmag'][obj], 0.001, 0)
...
def illum_combine():
    ...
    for i in np.arange(npatch):
        ...
        temp_so = ui.readDatafile('h%u'%heal_num+'star_obs.dat', so_keys,
                                  keytypes=('u8', 'u8', 'float', 'float', 'u8', 'u8'))
        temp_illum = ui.readDatafile('h%u'%heal_num+'_bestfit_FpPatch.dat',
                                     illum_keys, keytypes=('u8', 'float'))
        # now I need to construct a line for each unique patchid+illumid pair
        temp_so['patch_plus_illum'] = np.core.defchararray.add(temp_so['patchid'].astype('|S10'), np.core.defchararray.add(
            np.core.defchararray.replace(np.zeros(np.size(temp_so['patchid']), dtype='|S1'), '', ','), temp_so['illumid'].astype('|S10')))
        ...
        temp_so = np.core.records.fromarrays([temp_so['patchid'], temp_so['illumid'], temp_so['patchid']*0+heal_num, temp_so['starmag']*0, 1./np.sqrt(
            temp_so['nstars']), temp_so['starmag']*0], names=('patchid', 'illumid', 'healid', 'patchmag', 'patchmagerr', 'illummag'))
    ...
    for obj in range(0, len(patchdat['patchid'])):
        print >>file, "%d  %d  %f  %f %d"%(patchdat['healid'][obj], patchdat['patchid'][obj], patchdat['patchmag'][
                                           obj], patchdat['patchmagerr'][obj], patchdat['illumid'][obj])
...
def finalStar(infile='star_obs.dat', outfile='test_bestfit_Star.dat'):
    ...
    for i in np.arange(left.size):
        mag[i] = np.sum(star_obs['starmag'][left[i]:right[i]]/star_obs['starerr']
                        [left[i]:right[i]]**2)/np.sum(1./star_obs['starerr'][left[i]:right[i]]**2)
...
def finalStarP(infiles='h*star_obs.dat', outfile='test_bestfit_Star.dat', outstats='stats_stars'):
    ...
    for i in np.arange(np.size(files)):
        ...
        for i in np.arange(left.size):  # apply patch zeropoints
            star_obs['starmag'][left[i]:right[i]] = star_obs[
                'starmag'][left[i]:right[i]] - patchzp['patchzp'][i]
        ...
        for i in np.arange(left.size):
            star_obs['starmag'][left[i]:right[i]] = star_obs[
                'starmag'][left[i]:right[i]] - illumzp['illumzp'][i]
        ...
        for i in np.arange(left.size):
            ...
            temp_results['mag'][i] = np.sum(star_obs['starmag'][left[i]:right[
                                            i]]/star_obs['starerr'][left[i]:right[i]]**2)/np.sum(1./star_obs['starerr'][left[i]:right[i]]**2)
