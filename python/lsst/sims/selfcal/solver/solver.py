...
class lsqrSolver(object):
    ...
    def cleanData(self):
        ...
        for i in range(np.size(left)):
            self.observations['starID'][left[i]:right[i]] = Stars_index[i]
    ...
    def solveMatrix(self):
        ...
        col = np.append(self.observations['patchID'], self.observations[
                        'starID']+np.max(self.observations['patchID']))
        #data = np.append(np.ones(nObs),1./observations['magErr'])
        data = 1./self.observations['magErr']
        data = np.append(data, data)
        # maybe do this in place earlier?  then I can just delete parts of
        # observations earlier to save total memory
        b = self.observations['mag']/self.observations['magErr']
