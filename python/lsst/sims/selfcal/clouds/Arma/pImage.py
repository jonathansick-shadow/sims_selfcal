...
class PImage():
    ...
    def calcFft(self):
        ...
        # Note, these frequencies follow unshifted order (0= first, largest
        # spatial scale (with positive freq)).
    ...
    def calcSf(self):
        ...
        self.sfx = numpy.arange(
            0, (numpy.sqrt((self.nx/2.0 - self.padx)**2 + (self.ny/2.0 - self.pady)**2)), 1.0)
    ...
    def _makeRandomPhases(self, seed=None):
        ...
        # Set 0,0 to 0 to preserve overall mean value of image. (= 0 means no
        # value into imaginary component of FFT of image).
        self.phasespecI[0][0] = 0
        # Mirror upper left corner into lower right corner.
        self.phasespecI[self.xcen+1:self.nx, self.ycen+1:self.ny] = -1. * \
            numpy.fliplr(numpy.flipud(self.phasespecI[1:self.xcen, 1:self.ycen]))
        # Mirror upper right corner into lower left corner.
        self.phasespecI[1:self.xcen, self.ycen+1:self.ny] = -1. * \
            numpy.fliplr(numpy.flipud(self.phasespecI[self.xcen+1:self.nx, 1:self.ycen]))
        return
    ...
    def invertAcovf1d(self, acovfx=None, acovf1d=None, phasespec=None, seed=None, reduceNoise=True):
        ...
        # seems like this is unnecessary. So, unless reduceNoise is set to False,
        # just truncate acovf1d at rmax and extend with constant value.
