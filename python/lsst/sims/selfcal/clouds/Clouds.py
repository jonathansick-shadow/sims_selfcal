...
class Clouds:
    ...
    def __init__(self, ws, s):
        # set the half-diagonal size of the 2D Fourier space equal to the maximum
        # frequency of the 1D power spectrum
        self.windowsize = ws/numpy.sqrt(2.)
        # set the sampling directly in the real space : same sampling than the
        # correlation function (arbitrary)
        self.sampling = s
        # and the spacing
    ...
    def DirectClouds(self, correl2D=None, randomSeed=None):
        ...
        noise2D = numpy.random.normal(numpy.zeros(self.sampling*self.sampling),
